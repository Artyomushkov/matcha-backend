...
from profile.search_utils import get_user_sex_prefernces, is_get_params_valid_search, parse_data_to_frontend, get_age_range, get_fame_range, get_geo_range, create_order_condition, find_order_data
...
def search(page):
  ...
  try:
    ...
    order_data = find_order_data(request.json.get('order'), id)
  ...
  try:
    data = select_for_search(id, sex_info, age_range, fame_range, geo_range, request.json.get('tags'), page, order, order_data)
  except Exception as e:
    print(str(e))
    return jsonify({'error': "Database error"}), 500
